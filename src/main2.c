...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < line_no; i++) {
        ...
        case METAL:
            ...
            roma =
                get_roma(line_tokens[i].arr, 0, line_tokens[i].arr_len - 4,
                         assigned_pair, assigned_len);
        ...
        case MUCH:
            roma =
                get_roma(line_tokens[i].arr, 3, line_tokens[i].arr_len - 4,
                         assigned_pair, assigned_len);
            ...
            much =
                answer_begin(line_tokens[i].arr, 3, line_tokens[i].arr_len - 4);
        ...
        case MANY:
            roma =
                get_roma(line_tokens[i].arr, 4, line_tokens[i].arr_len - 5,
                         assigned_pair, assigned_len);
            if (check_roma(roma) != 0) {
                sprintf(answer_line[answer_len++], "%s", OUTPUT_INVALID_ROMA);
                free(roma);
            ...
            for (j = 0; j < metal_len; j++) {
                if (strcmp(line_tokens[i].arr[metal_no], metal_pair[j].metal) ==
                    0) {
                    break;
                }
            }
            ...
            many =
                answer_begin(line_tokens[i].arr, 4, line_tokens[i].arr_len - 6);
            sprintf(answer_line[answer_len++], "%s%s is %.2f Credits", many,
                    line_tokens[i].arr[metal_no], credits);
            free(roma);
            free(many);
            break;
